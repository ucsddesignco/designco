...
function createOperationToPatches(blockContentFeatures, blockContentType) {
  ...
  function removeNodePatch(operation, beforeValue, afterValue) {
    var patches = [];
    var isPlaceholder = beforeValue.document.nodes.get(operation.path.get(0)).data.get('placeholder');
    var block = toBlock(beforeValue, operation.path.get(0)); // Send unset patch unless is it is a placeholder
